...
<table class="table table-striped table-bordered table-hover">
    ...
    <tbody>
        @if ($details->count() > 0)
            @foreach ($details as $detail)
                    <td><input type="checkbox" class="booking-checkbox" value="{{ $detail->id }}"></td>
    ...
    <!-- jQuery Core (latest 3.x version) -->
...
$(document).ready(function(){
    ...
    $('.booking-checkbox').click(function(){
        $('#selectAll').prop('checked', $('.booking-checkbox:checked').length === $('.booking-checkbox').length);
    });
});
</script>
</table>
